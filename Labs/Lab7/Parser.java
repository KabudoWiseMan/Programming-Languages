/**
 * Created by vsevolodmolchanov on 04.04.17.
 */
...
public class Parser {
    ...
    private static void parseChain() throws SyntaxError {
        if (sym.matches(Tag.IDENT)) {
            System.out.println("<Chain> ::= IDENT <Tail>");
            sym = sym.next();
            parseTail();
        } else {
    ...
    private static void parseTail() throws SyntaxError {
        if (sym.matches(Tag.DOT)) {
            System.out.println("<Tail> ::= . IDENT ( <Arg> ) <Tail>");
            sym = sym.next();
            if(sym.matches(Tag.IDENT)) {
                sym = sym.next();
                expect(Tag.LPAREN);
